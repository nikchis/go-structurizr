...
import (
	...
	"github.com/nikchis/go-structurizr/pkg/internal"
	"github.com/nikchis/go-structurizr/pkg/internal/test"
	"github.com/nikchis/go-structurizr/pkg/model"
	"github.com/nikchis/go-structurizr/pkg/scraper"
	"github.com/stretchr/testify/require"
)

const (
	testPKG = "github.com/nikchis/go-structurizr/pkg/internal/test"
)
...
func TestScraper_Scrape_package_matching(t *testing.T) {
	var tests = []struct {
		...
		},
		{
			...
			structure: test.NewRootEmptyHasInfo(),
			packages: []string{
				"github.com/nikchis/go-structurizr/pkg/foo",
			},
		...
		},
		{
			...
			structure: test.NewRootEmptyHasInfo(),
			packages: []string{
				"github.com/nikchis/go-structurizr/pkg",
			},
			expectedNumberOfComponents: 1,
		},
	}
...
func TestScraper_Scrape_rules(t *testing.T) {
	...
	ruleMatchPublicComponentInAnotherPackage, err := scraper.NewRule().
		WithPkgRegexps("^github.com/nikchis/go-structurizr/pkg/foo$").
		WithNameRegexp("^test.PublicComponent$").
		WithApplyFunc(func(name string, groups ...string) model.Info {
			return model.ComponentInfo()
		}).
